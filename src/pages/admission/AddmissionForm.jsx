...
const AddmissionFrom = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (!userPhotoFile) {
        setSubmitMessage({ type: "error", text: "Student photo is required" });
        return;
      }

      if (documents.length === 0) {
        setSubmitMessage({ type: "error", text: "At least one document is required" });
        return;
      }
      ...
      // Add student profile fields (flat keys for backend parsing)
      formData.append("firstName", data.studentFirstName || "");
      formData.append("lastName", data.studentLastName || "");
      ...
      const academic = {
        currentClass: data.applyingForClass || "",
        session: data.academicSession || "",
        currentSection: data.Section || "",
        previousSchool: {
          name: data.previousSchoolName || "",
          lastClass: data.lastClassPassed || "",
        },
      };
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <Button
            className="flex gap-2 bg-primary-500 "
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 px-4">
            ...
            <div>
              <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                <h3 className="text-2xl font-bold text-gray-900">Parents Information</h3>
                <label className="flex items-center gap-2 text-base font-medium text-gray-600 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked={hasParents}
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-3">
                ...
                <span className="text-base font-normal text-red-500">(Required)</span>
              </h3>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Input
                  className="bg-gray-200 border-2 border-gray-300"
                  ...
                  {...register("guardianName", { required: !hasParents ? "Guardian name is required" : false })}
                />
                <Input
                  className="bg-gray-200 border-2 border-gray-300"
                  label="CNIC"
                ...
                <Input
                  ...
                  {...register("guardianContact", { required: !hasParents ? "Guardian contact is required" : false })}
                />
              ...
              <div className="mt-2">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div>
                    <label className="block text-lg font-medium text-gray-700 mb-1">
                      Applying for Class<span className="text-red-500 ml-1">*</span>
                    </label>
                    <select
                      className="w-full rounded-lg border-2 border-gray-300 px-4 py-2 text-sm bg-gray-200 focus:outline-none focus:ring-2 focus:ring-primary-500"
                      {...register("applyingForClass", { required: "Class is required" })}
                    >
                    ...
                    {errors.applyingForClass && (
                      <p className="mt-1 text-xs text-red-500">{errors.applyingForClass.message}</p>
                    )}
                  </div>

                  <div>
                    <label className="block text-lg font-medium text-gray-700 mb-1">
                      Academic Session<span className="text-red-500 ml-1">*</span>
                    </label>
                    <select
                      className="w-full rounded-lg border-2 border-gray-300 px-4 py-2 text-sm bg-gray-200 focus:outline-none focus:ring-2 focus:ring-primary-500"
                    ...
                    {errors.academicSession && (
                      <p className="mt-1 text-xs text-red-500">{errors.academicSession.message}</p>
                    )}
                  </div>
              ...
              <div className="mt-6">
                ...
                {documents.length > 0 && (
                  <div className="mt-4 space-y-2">
                    <h4 className="font-semibold text-gray-700">Selected Documents ({documents.length})</h4>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
              ...
              <Button
                type="submit"
                className="px-6 bg-primary-500 rounded disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                disabled={isSubmitting}
              >
                {isSubmitting ? (
                  <>
